...
# 手动指定每一次的batch normalization在train过程还是test过程中

# tf.layers.conv2d_transpose本质还是一个卷积运算
# 只是为了使得输出的数据的维度和之前做正向卷积的时候相同
# 输出的维度根据input和stride等信息自行计算或是output_shape来指定
# 根据正向时的依赖关系计算相关位置上的数值
...
def discriminator(x, reuse=False):
    with tf.variable_scope('Discriminator', reuse=reuse):
        ...
        x = tf.layers.conv2d(x, 64, 5, strides=2, padding='same')  #(batch_size, 14, 14, 64)
        x = tf.layers.batch_normalization(x, training=is_training)
        x = leakyrelu(x)
        x = tf.layers.conv2d(x, 128, 5, strides=2, padding='same')   # (batch_size, 7, 7, 128)
        x = tf.layers.batch_normalization(x, training=is_training)
        x = leakyrelu(x)
        # Flatten
        x = tf.reshape(x, shape=[-1, 7*7*128])
...
# 在model中添加bn时，train过程会使用滑动平均来对每一个batch求取mean和variance
# 作为一种对于整体训练集的概括
# 这些参数在test会直接被使用而不去计算当前batch的mean和variance
...
# 确保对mean和variance的操作会在optimization之前完成
# 即每一次进行minimize操作都执行一次bn层中的滑动平均的update
